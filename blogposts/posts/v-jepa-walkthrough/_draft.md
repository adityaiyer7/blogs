...
<iframe
  id="ijepa-crop-artifact"
  src="interactive/i-jepa/dist/index.html?embed=true&amp;artifact=crop"
  title="Interactively sample an I-JEPA crop from the source image"
  loading="lazy"
  sandbox="allow-scripts allow-same-origin"
  width="100%"
  height="850"
  style="border: 1px solid #d7d9dc; border-radius: 8px;"
></iframe>

<script>
(() => {
  const cropFrame = document.getElementById("ijepa-crop-artifact");
  if (!cropFrame) return;

  const defaults = {area: "0.50", aspect: "1.00", position: "0"};
  let lastState = "";

  const syncCropState = () => {
    try {
      const cropUrl = new URL(cropFrame.contentWindow.location.href);
      const state = Object.fromEntries(
        Object.entries(defaults).map(([key, fallback]) => [
          key,
          cropUrl.searchParams.get(key) ?? fallback,
        ])
      );
      const stateKey = JSON.stringify(state);
      if (stateKey === lastState) return;

      document.querySelectorAll(".ijepa-derived-artifact").forEach((frame) => {
      ...
      // The frame may not be ready yet; the next interval will retry.
    }
  };

  cropFrame.addEventListener("load", syncCropState);
  const syncTimer = window.setInterval(syncCropState, 400);
...
<iframe
  class="ijepa-derived-artifact"
  data-artifact="resize"
  src="interactive/i-jepa/dist/index.html?embed=true&amp;artifact=resize"
  title="Visualize an I-JEPA crop being resized to 224 by 224 pixels"
  loading="lazy"
  sandbox="allow-scripts allow-same-origin"
  width="100%"
  height="560"
...
<iframe
  class="ijepa-derived-artifact"
  data-artifact="patches"
  src="interactive/i-jepa/dist/index.html?embed=true&amp;artifact=patches"
  title="Visualize the resized I-JEPA image as a 16 by 16 patch grid"
  loading="lazy"
  sandbox="allow-scripts allow-same-origin"
  width="100%"
  height="620"
...
<iframe
  class="ijepa-derived-artifact"
  data-artifact="masks"
  src="interactive/i-jepa/dist/index.html?embed=true&amp;artifact=masks"
  title="Interactively sample I-JEPA target blocks and retained context"
  loading="lazy"
  sandbox="allow-scripts allow-same-origin"
  width="100%"
  height="850"
